...
def main():
    ...
    parser.add_argument("--index_file", type=str, required=True, help="File of indices")
    ...
    # Read index file to get image indices
    with open(args.index_file, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if not line:
                continue
            try:
                image_indices.append(int(line))
            except (ValueError, IndexError):
                print(f"Skipping malformed line in index file: {line}")
                continue
